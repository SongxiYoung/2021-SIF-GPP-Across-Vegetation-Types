%% SCOPE_PROSPECT_SIF_SIMULATION.m (script)
...
% 0: individual runs (specify all input in a this file)
% 1: time series (uses text files with meteo input as time series)
% 2: Lookup-Table (specify the values to be included)
% 3: Lookup-Table with random input (specify the ranges of values)

% T = shaperead('2020071/YRR_LUT.shp') ; 
T = shaperead('AlakesR1909_LUT.shp') ; 
...
%     V(5).Val = input_var(6);      % Cs->Senescence factor in PROSPECT
      V(1).Val = T(i).chl;      % Cab->Chlorophyll content in PROSPECT
      V(2).Val = V(1).Val*0.25;       %Cca->Carotenoid content in PROSPECT
...
%     V(33).Val  = input_var(17);       % p->Air pressure
      V(35).Val  = T.SPEED;             % u->Wind speed
%     V(37).Val  = input_var(19);       % Oa->O2 concentration in the air
      V(34).Val  = T.VP;                % ea->Atmospheric vapor pressure
%     V(36).Val  = input_var(21);       % Ca->CO2 concentration in the air
      V(31).Val  = T.TLML;              % Ta->Air temperature
      V(31).Val  = V(31).Val - 273.15;
      V(30).Val  = T.SWGDN;             % Rin->Incoming shortwave radiation
      V(32).Val  = T.LWGAB;             % Rli->Incoming longwave radiation
...
%     V(27).Val = input_var(27);      % LIDFb->LIDF parameter b, which controls the distribution's bimodality
      V(22).Val = T(i).MOD_LAI * 0.1; % LAI->Leaf area index
...
%     V(53).Val  = input_var(32);     % RAA->Relative azimuth angle
    ...
    T(i).SIFobs_740 = rad.LoF_(101);
    T(i).SIFobs_757 = rad.LoF_(118);
    T(i).SIFobs_771 = rad.LoF_(132); 
    T(i).SIFtot_740 = rad.LoF_(101);
    T(i).SIFtot_757 = rad.LoF_(118);
    T(i).SIFtot_771 = rad.LoF_(132);
    T(i).PAR = canopy.Rntot_PAR;
    
end

%% 15. output
% shapewrite(T,'mySifData_Asia3.shp');
shapewrite(T,'SynSif_AlakesR1909.shp');
